...
def getData(req, cmd):
    print(req+cmd)
    #print(toHexString(req + cmd))
    data, sw1, sw2 = connection.transmit(req + cmd)
    return [data, sw1, sw2]
...
if (atr[0] == 0x3B & atr[1] == 0x7f):
    req = [0x00, 0xA4, 0x04, 0x00]
else :
    req = [0x00, 0xc0, 0x00, 0x00]
req = [0x00, 0xA4, 0x04, 0x00]
# Check card
data, sw1, sw2 = connection.transmit(SELECT + UY_CARD)
print("Select Applet: %02X %02X" % (sw1, sw2))

# CID
data = getData(req, CMD_CID)
print(data)
...
pin = [hex(ord(c)) for c in '205712100000']
data = getData(['0x00', '0x20', '0x00', '0x11', '0x0c'], pin)
print('pin:', data)
#
# # TH Fullname
